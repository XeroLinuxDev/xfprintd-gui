...
/// Checks if the current distribution is supported and shows error dialog if not.
pub fn check_distribution_support(main_window: &ApplicationWindow) {
    ...
    if !util::is_supported_distribution() {
        ...
        // Set the current distribution name
        distro_label.set_label(&format!("Current distribution: {}", distro_name));

        // Set the main window as transient parent
        error_window.set_transient_for(Some(main_window));

        // Connect OK button to close both windows
        ...
        .output()
    {
        Ok(output) => {
            ...
            if status == "active" {
                ...
                log::warn!("⚠️  fprintd service status: {}", status);
                log::warn!("   You may need to start fprintd: sudo systemctl start fprintd");
            }
        }
        Err(e) => {
            log::warn!("⚠️  Cannot check fprintd service status: {}", e);
        }
    }
}

/// Checks for helper tool availability.
pub fn check_helper_tool() {
    ...
    if std::path::Path::new(helper_path).exists() {
        ...
        log::warn!("⚠️  Helper tool not found at: {}", helper_path);
        log::warn!("   PAM configuration features may not work");
    }
}

/// Checks for pkexec availability.
    ...
    match std::process::Command::new("which").arg("pkexec").output() {
        Ok(output) => {
            if output.status.success() {
                ...
                log::warn!("⚠️  pkexec not found - PAM configuration will not work");
            }
        }
        Err(_) => {
            log::warn!("⚠️  Cannot check for pkexec availability");
        }
    }
}
